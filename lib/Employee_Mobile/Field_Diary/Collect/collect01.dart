...
class _CollectStep1ScreenState extends State<CollectStep1Screen> {
  ...
  // This key forces a visual reset of all text fields when the form is cleared
  ...
  void _handleClearAll(ObservationModel model) {
    setState(() {
      // 1. Reset the data in the global Provider (Clears Step 1, 2, and 3)
      model.reset();
      
      // 2. Generate a new key to physically wipe the TextFields on this screen
      _formKey = UniqueKey();
    });
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: ListView(
              key: _formKey, // Visual reset happens here
          ...
          TextButton(
            onPressed: () => showDialog(
              ...
              builder: (context) => ClearEntryDialog(
                onClear: () => _handleClearAll(model), // Fixed: used 'onClear' to match your dialog
              ),
            ), 
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildMemberInput("Firstname", "Enter firstname", (v) => model.members[index]['firstname'] = v),
          const SizedBox(height: 12),
          _buildMemberInput("Lastname", "Enter lastname", (v) => model.members[index]['lastname'] = v),
          const SizedBox(height: 12),
          _buildMemberInput("User Role", "e.g. Field Observer", (v) => model.members[index]['role'] = v),
        ],
      ),
    );
  }
  ...
  Widget _buildMemberInput(String label, String placeholder, Function(String) onChanged) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
