...
class _MeetingsScreenState extends State<MeetingsScreen> {
  ...
  int _activeFilterIndex =
      4; // Default to Upcoming (Index 4) so upcoming meetings display first
  String _searchQuery = "";
      ...
      body: CustomScrollView(
        slivers: [
          ...
          StreamBuilder<List<Map<String, dynamic>>>(
            stream: _supabase
                .from('meetings')
                .stream(primaryKey: ['id']).order('meeting_date'),
              ...
              return StreamBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, rsvpSnapshot) {
                  ...
                  final consolidated = meetings.where((m) {
                    // Role Filtering
                    String rolesString = (m['target_roles'] ?? '')
                        .toString()
                        .toLowerCase()
                        .replaceAll(' ', '');
                    bool matchesRole = rolesString.contains('fieldofficer') ||
                        rolesString.contains('all') ||
                        rolesString.isEmpty;

                    // Search Filtering
                    bool matchesSearch = _searchQuery == "" ||
                        (m['title'] ?? '')
                            .toString()
                            .toLowerCase()
                    ...
                    final rsvp = userRSVPs.firstWhere(
                        (r) => r['meeting_id'] == m['id'],
                  ...
                  final futureMeetings =
                      consolidated.where((m) => !_isMeetingDone(m)).toList();
                  final completedMeetings =
                      consolidated.where((m) => _isMeetingDone(m)).toList();

                  final upCount = futureMeetings.length;
                  final reCount = futureMeetings
                      .where((m) => m['is_mandatory'] == true)
                      .length;
                  ...
                  final filtered = consolidated.where((m) {
                    ...
                    if (_activeFilterIndex == 4)
                      return !isDone; // Upcoming Meetings (Default)
                    if (_activeFilterIndex == 1)
                      return !isDone &&
                          m['is_mandatory'] == true; // Upcoming Required
                    if (_activeFilterIndex == 2)
                      return !isDone &&
                          m['user_status'] == 'attending'; // Upcoming Attending
                    if (_activeFilterIndex == 3)
                      return !isDone &&
                          m['user_status'] == 'declined'; // Upcoming Declined
                    if (_activeFilterIndex == 5)
                      return isDone; // Completed / Done Meetings
                  ...
                  return SliverPadding(
                    ...
                    sliver: SliverList(
                      delegate: SliverChildListDelegate([
                        ...
                        const SizedBox(height: 20),
                        _buildSectionHeader(
                            _getFilterTitle(), isDark, textTheme),
                        if (filtered.isEmpty)
                          Padding(
                          ...
                          ...filtered
                              .map((m) => _buildMeetingCard(m, isDark))
                              .toList(),
                      ]),
                    ),
                  );
      ...
      SliverAppBar(
        ...
        leading: Padding(
          ...
          child: Image.asset('assets/logo2.png',
              fit: BoxFit.contain,
              errorBuilder: (c, e, s) =>
                  Icon(Icons.eco, color: forestGreen, size: 32)),
        ),
        title: Text("Meetings",
  ...
  Widget _buildKPIRow(int up, int rs, bool d) => Row(children: [
        Expanded(
            child: _statCard("$up", "Meetings", d, forestGreen,
                Icons.event_available_rounded, 4)),
        const SizedBox(width: 14),
        Expanded(
            child: _statCard("$rs", "Required", d, accentOrange,
                Icons.priority_high_rounded, 1)),
      ]);

  Widget _statCard(
      String val, String lbl, bool d, Color c, IconData icon, int index) {
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: isActive
                  ? c.withOpacity(0.6)
                  : (d ? Colors.white10 : Colors.black12),
              width: isActive ? 2 : 1),
          boxShadow: [
            BoxShadow(
              color: isActive
  ...
  Widget _buildFilterRow(bool d) => SingleChildScrollView(
        ...
        child: Row(children: [
          _filtBtn("Upcoming", 4, Icons.schedule_rounded, d),
          const SizedBox(width: 8),
          _filtBtn("Required", 1, Icons.stars_rounded, d),
          const SizedBox(width: 8),
          _filtBtn("Attending", 2, Icons.check_circle_outline_rounded, d),
          const SizedBox(width: 8),
          _filtBtn("Not Attending", 3, Icons.cancel_outlined, d),
          const SizedBox(width: 8),
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSel
              ? forestGreen
              : (d ? const Color(0xFF1F1F1F) : Colors.white),
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
              color:
                  isSel ? forestGreen : (d ? Colors.white12 : Colors.black12),
              width: 1),
          boxShadow: isSel
              ? [
                  BoxShadow(
        ...
        child: Row(
          ...
          children: [
            Icon(icon,
                ...
                color: isSel
                    ? Colors.white
                    : (d ? Colors.white70 : Colors.black54)),
            ...
            Text(
              ...
              style: TextStyle(
                ...
                color: isSel
                    ? Colors.white
                    : (d ? Colors.white70 : Colors.black87),
  ...
  Widget _buildSectionHeader(String title, bool isDark, TextTheme textTheme) =>
      Padding(
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 14,
              decoration: BoxDecoration(
                  color: forestGreen, borderRadius: BorderRadius.circular(2)),
            ),
            const SizedBox(width: 8),
            Text(title,
  ...
  Widget _buildMeetingCard(Map<String, dynamic> m, bool d) {
    ...
    final bool hasMom =
        (m['minutes'] != null && m['minutes'].toString().trim().isNotEmpty) ||
            (m['mom_attachment_url'] != null &&
                m['mom_attachment_url'].toString().trim().isNotEmpty);

    final bool isDone = _isMeetingDone(m);

    String badgeTxt = status == 'attending'
        ? "Attending"
        : (status == 'declined' ? "Declined" : "RSVP Required");
    Color badgeCol = status == 'attending'
        ? forestGreen
        : (status == 'declined' ? errorRed : accentOrange);
    IconData badgeIcon = status == 'attending'
    ...
    final Color accentStripColor =
        isDone ? Colors.grey : (isMandatory ? accentOrange : forestGreen);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
            color: d ? Colors.white10 : Colors.black.withOpacity(0.06),
            width: 1),
        boxShadow: [
          BoxShadow(
              ...
              Expanded(
                child: InkWell(
                  onTap: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => MeetingViewScreen(meeting: m))),
                  child: Padding(
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Icon(Icons.chevron_right_rounded,
                                size: 22,
                                color: d ? Colors.white38 : Colors.black26),
                          ],
                        ),
                        ...
                        Row(
                          children: [
                            ...
                            Text("${m['meeting_date']}",
                                style: TextStyle(
                                    fontSize: 11,
                                    color: d ? Colors.white70 : Colors.black54,
                                    fontWeight: FontWeight.w500)),
                            if (m['meeting_time'] != null &&
                                m['meeting_time'].toString().isNotEmpty) ...[
                              Text("  •  ",
                                  style: TextStyle(
                                      fontSize: 11,
                                      color:
                                          d ? Colors.white30 : Colors.black26)),
                              ...
                              Expanded(
                                child: Text(
                                  ...
                                  style: TextStyle(
                                      fontSize: 11,
                                      color:
                                          d ? Colors.white54 : Colors.black45),
                                  maxLines: 1,
                        ...
                        if (hasMom) ...[
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color:
                                      forestGreen.withOpacity(d ? 0.3 : 0.2)),
                            ),
                            child: Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                    color: badgeCol.withOpacity(0.3)),
                              ),
                              child: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
