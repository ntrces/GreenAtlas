...
class EmployeePortal extends StatefulWidget {
  // FIXED: Added initialIndex to allow navigating back to a specific tab
...
class _EmployeePortalState extends State<EmployeePortal> {
  // FIXED: Changed to late so we can initialize from widget property
  ...
  void initState() {
    super.initState();
    // FIXED: Initialize starting tab based on the passed index
...
class _EmployeeDashboardContentState extends State<EmployeeDashboardContent> {
        ...
        SliverPadding(
          ...
          sliver: SliverList(
            delegate: SliverChildListDelegate([
              ...
              if (_isMeetingsExpanded) _buildUpcomingMeetingsList(isDark),
      ...
      children: [
        Expanded(
          child: StreamBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              return _buildLargeStatCard(Icons.menu_book_outlined, "$count", "Diary Entries", isDark, onTap: () => widget.onNavigate(1));
            },
          ),
        ),
        ...
        Expanded(
          child: StreamBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              final count = snapshot.data?.length ?? 0;
              return _buildLargeStatCard(Icons.calendar_month_outlined, "$count", "Meetings", isDark, onTap: () => widget.onNavigate(2));
            },
          ),
        ),
      ],
  ...
  Widget _buildLargeStatCard(IconData icon, String value, String label, bool isDark, {VoidCallback? onTap}) => InkWell(
    onTap: onTap,
    borderRadius: BorderRadius.circular(12),
    child: Container(
      padding: const EdgeInsets.symmetric(vertical: 24),
      decoration: BoxDecoration(color: isDark ? const Color(0xFF1F1F1F) : Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 8, offset: const Offset(0, 4))]),
      child: Column(children: [
          Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: isDark ? Colors.white10 : const Color(0xFFF0F4F0), shape: BoxShape.circle), child: Icon(icon, color: isDark ? Colors.white70 : Colors.black45, size: 24)),
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return Column(children: snapshot.data!.map((e) {
            ...
            return _buildStatusRow(e['taxon'] ?? 'Observation', "${e['location'] ?? 'Area'} • ${DateFormat('jm').format(date)}", status, status == 'Validated' ? Colors.green : Colors.orange, isDark, true, onTap: () => widget.onNavigate(1));
        }).toList());
      },
    );
  }
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return Column(children: snapshot.data!.map((m) {
            ...
            return _buildStatusRow(m['title'] ?? 'Meeting', DateFormat('MMM d, yyyy').format(date), m['status'] ?? 'Scheduled', const Color(0xFF5D7A5D), isDark, false, onTap: () => widget.onNavigate(2));
        }).toList());
      },
    );
  }
  ...
  Widget _buildLiveDiaryAction(bool isDark) {
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return _buildDetailedAction(icon: Icons.menu_book_outlined, title: "Field Diary", subtitle: "Document observations", badge: pending > 0 ? "$pending Pending" : null, badgeColor: Colors.orange, isDark: isDark, onTap: () => widget.onNavigate(1));
      }
    );
  }
  ...
  Widget _buildLiveMeetingAction(bool isDark) {
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        final count = snapshot.data?.length ?? 0;
        return _buildDetailedAction(icon: Icons.calendar_today_outlined, title: "Meetings", subtitle: "View schedule & RSVP", badge: count > 0 ? "$count New" : null, badgeColor: const Color(0xFF5D7A5D), isDark: isDark, onTap: () => widget.onNavigate(2));
      }
    );
  }

  Widget _buildEmptyState(bool isDark, String msg) => Container(width: double.infinity, padding: const EdgeInsets.all(20), color: isDark ? const Color(0xFF1F1F1F) : Colors.white, child: Text(msg, style: const TextStyle(fontSize: 12, color: Colors.grey)));
  ...
  Widget _buildResponsiveHeader(BuildContext context, bool isDark) => SliverAppBar(
    ...
    actions: [_buildNotificationIcon(context, isDark), _buildTopProfileIcon(context, isDark), const SizedBox(width: 16)],
  );

  Widget _buildNotificationIcon(BuildContext context, bool isDark) => Stack(alignment: Alignment.center, children: [
      ...
      Positioned(right: 8, top: 18, child: Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: Color(0xFF5D7A5D), shape: BoxShape.circle), child: const Text("2", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))))
  ]);
  ...
  Widget _buildSectionHeader(String title, bool isDark, {String? trailing, VoidCallback? onTrailingTap, bool hasDropdown = false, bool isExpanded = true, VoidCallback? onDropdownTap}) => Padding(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      if (trailing != null) InkWell(onTap: onTrailingTap, child: Text(trailing, style: const TextStyle(fontSize: 12, color: Color(0xFF5D7A5D), fontWeight: FontWeight.bold))),
  ...
  Widget _buildDetailedAction({required IconData icon, required String title, required String subtitle, required bool isDark, required VoidCallback onTap, String? badge, Color? badgeColor}) => Container(
    ...
    child: ListTile(onTap: onTap, contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 4), leading: Icon(icon, color: isDark ? Colors.white70 : Colors.black45, size: 22), title: Text(title, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: isDark ? Colors.white : Colors.black87)), subtitle: subtitle.isEmpty ? null : Text(subtitle, style: TextStyle(fontSize: 12, color: isDark ? Colors.white38 : Colors.black38)), trailing: Row(mainAxisSize: MainAxisSize.min, children: [if (badge != null) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: badgeColor!.withOpacity(0.12), borderRadius: BorderRadius.circular(10)), child: Text(badge, style: TextStyle(color: badgeColor, fontSize: 10, fontWeight: FontWeight.bold))), const SizedBox(width: 8), const Icon(Icons.chevron_right, size: 20, color: Colors.black26)])),
  );

  Widget _buildStatusRow(String title, String subtitle, String status, Color color, bool isDark, bool isEntry, {VoidCallback? onTap}) => Container(
    margin: const EdgeInsets.only(bottom: 1), width: double.infinity,
    decoration: BoxDecoration(color: isDark ? const Color(0xFF1F1F1F) : Colors.white),
    child: ListTile(onTap: onTap, contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 4), leading: Icon(isEntry ? (status == "Validated" ? Icons.check_circle_outline : Icons.access_time) : Icons.calendar_month_outlined, color: isEntry ? color : (isDark ? Colors.white24 : Colors.black26), size: 24), title: Text(title, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: isDark ? Colors.white : Colors.black87)), subtitle: Text(subtitle, style: TextStyle(fontSize: 12, color: isDark ? Colors.white38 : Colors.black38)), trailing: Row(mainAxisSize: MainAxisSize.min, children: [Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(6)), child: Text(status, style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.bold))), const SizedBox(width: 8), const Icon(Icons.chevron_right, size: 20, color: Colors.black12)])),
  );
}