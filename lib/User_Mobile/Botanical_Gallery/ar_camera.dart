...
class _ARCameraScreenState extends State<ARCameraScreen> {
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        d['description'] ?? "", 
                        style: textTheme.bodySmall?.copyWith(color: Colors.black87, height: 1.5, fontSize: 12),
                      ),
  ...
  Widget _buildSmallHeader(String? n, String? s, String? c, TextTheme textTheme) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(n ?? "Unknown", style: textTheme.titleMedium?.copyWith(fontSize: 17, fontWeight: FontWeight.bold)),
        Text(s ?? "N/A", style: textTheme.labelSmall?.copyWith(color: Colors.black45, fontSize: 11, fontStyle: FontStyle.italic)),
      ])),
      Container(
  ...
  Widget _buildConservationStatus(String? s, String? source, TextTheme textTheme) {
    ...
    if (status.toLowerCase().contains("least concern") || status.toLowerCase().contains("not threatened")) {
      bgColor = Colors.green;
      cardBg = const Color(0xFFF0FDF4);
      borderColor = Colors.green[200]!;
    } else if (status.toLowerCase().contains("vulnerable") || status.toLowerCase().contains("near threatened")) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            const Icon(Icons.shield_outlined, size: 16, color: Color(0xFF5D7A5D)),
            const SizedBox(width: 8),
            Text("CONSERVATION STATUS", style: textTheme.labelSmall?.copyWith(fontSize: 11, color: const Color(0xFF5D7A5D), fontWeight: FontWeight.bold)),
          ],
        ),
              ...
              Row(
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      const Text("SOURCE", style: TextStyle(fontSize: 8, color: Colors.black54)),
                      Text(src, style: const TextStyle(fontSize: 10, color: Colors.black, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ],
              ),
              if (status.toLowerCase().contains("endangered")) ...[
                ...
                RichText(
                  text: TextSpan(
                    ...
                    children: [
                      const TextSpan(text: "Protected Species: ", style: TextStyle(fontWeight: FontWeight.bold)),
                      TextSpan(text: "This plant is ${status.toLowerCase()} and protected under Philippine environmental laws (RA 9147 & RA 11038). Unauthorized collection or harm is prohibited.")
                    ]
                  )
                )
              ]
  ...
  Widget _buildScientificClassification(Map d, TextTheme textTheme) => Column(children: [
    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      GestureDetector(
        onTap: () => setState(() => _isClassificationExpanded = !_isClassificationExpanded),
        child: Padding(
          padding: const EdgeInsets.only(bottom: 12),
          child: Row(
            children: [
              Text(_isClassificationExpanded ? "Hide" : "Show", style: const TextStyle(fontSize: 10, color: Colors.black54)),
              Icon(_isClassificationExpanded ? Icons.expand_less : Icons.expand_more, size: 16, color: Colors.black54),
            ],
          ),
        ),
      )
  ...
  Widget _row(String l, String v, TextTheme textTheme, bool isItalic) => Padding(
    padding: const EdgeInsets.symmetric(vertical: 8), 
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Text(l, style: textTheme.labelSmall?.copyWith(fontSize: 11, color: Colors.black45)), 
      Text(v, style: textTheme.titleSmall?.copyWith(fontStyle: isItalic ? FontStyle.italic : FontStyle.normal, fontSize: 13))
    ])
  );
  ...
  Widget _info(IconData i, String l, String v, TextTheme textTheme) => Padding(
    ...
    child: Row(children: [
      Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFE8F3E8), borderRadius: BorderRadius.circular(6)),
  ...
  Widget _buildEcologicalImportance(String text, TextTheme textTheme) => ClipRRect(
    borderRadius: BorderRadius.circular(8),
    child: Container(
      padding: const EdgeInsets.all(16), 
      decoration: const BoxDecoration(
        color: Color(0xFFE8F3E8),
        border: Border(left: BorderSide(color: Color(0xFF5D7A5D), width: 4))
      ),
      child: Text(text, style: textTheme.bodySmall?.copyWith(fontSize: 12, height: 1.5, color: Colors.black87))
    )
  );
