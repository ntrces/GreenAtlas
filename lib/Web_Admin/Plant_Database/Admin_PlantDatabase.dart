...
class _PlantDatabaseViewState extends State<PlantDatabaseView> {
                  ...
                  child: Row(
                    children: [
                      ClipRRect(
                        ...
                        child: Container(
                          width: 44, height: 44, color: Colors.black.withOpacity(0.05),
                          child: (plant['image_url'] != null && plant['image_url'].toString().isNotEmpty)
                            ? Image.network(plant['image_url'], fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.image_not_supported, size: 20, color: Colors.grey))
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
                  ...
                  child: SizedBox(
                    ...
                    child: AspectRatio(
                      ...
                      child: ClipRRect(
                        ...
                        child: Image.network(plant['image_url'], fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.broken_image)),
                      ),
                    ),
                  ),
                ),
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(
                    ...
                    children: [
                      Expanded(child: Text(plant['common_name'] ?? "No Name", style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF2D3E2D)))),
                      // ADDED: Connected Edit Button
                      IconButton(
                        onPressed: () => showDialog(context: context, builder: (_) => EditPlantDialog(plant: plant)),
                        icon: const Icon(Icons.edit_outlined, color: Colors.black45),
                      ),
                    ],
                  ),
                  const SizedBox(height: 4),
                  Text(plant['scientific_name'] ?? "Scientific Name Not Provided", style: const TextStyle(fontSize: 14, fontStyle: FontStyle.italic, color: Colors.black38)),
          ...
          _detailBlock("Description", plant['description'] ?? "No description available."),
          const SizedBox(height: 24),
          Row(children: [
            const Icon(Icons.view_in_ar, size: 18, color: Colors.black38),
            const SizedBox(width: 8),
            Expanded(child: Text(plant['ar_model_url'] ?? "No 3D Model Attached", style: const TextStyle(fontSize: 12, color: Colors.black38))),
          ]),
        ],
      ),
    );
  ...
  // --- HELPERS (UNCHANGED) ---
  Widget _buildHeader() => Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("PLANT DATABASE MANAGEMENT", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF2D3E2D))), Text("Species profiles • AR content • Educational metadata", style: TextStyle(fontSize: 13, color: Colors.black38))]), ElevatedButton.icon(onPressed: () => showDialog(context: context, builder: (context) => const AddSpeciesDialog()), icon: const Icon(Icons.add, size: 18), label: const Text("ADD SPECIES", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF4D6D4D), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(4))))]);
  Widget _buildMetricRow(List<Map<String, dynamic>> plants) { final endangered = plants.where((p) => p['conservation_status'] == 'Endangered').length; final arModels = plants.where((p) => p['ar_model_url'] != null && p['ar_model_url'].toString().isNotEmpty).length; return Row(children: [_metricCard(plants.length.toString(), "Total Species", "In database", Colors.black87), _metricCard(arModels.toString(), "AR Models", "3D assets", Colors.black87), _metricCard(endangered.toString(), "Endangered", "PRIORITY", Colors.red)]); }
